# カテゴリーデータを削除した上でデータ投入する場合、「Category.delete_all」を実行させる
# Category.delete_all

# 親カテゴリー
lady = Category.create(:name=>'レディース')
man = Category.create(:name=>'メンズ')
babykid = Category.create(:name=>'ベビー・キッズ')
interior = Category.create(:name=>'インテリア・住まい・小物')
bookmusicgame = Category.create(:name=>'本・音楽・ゲーム')
toyhobby = Category.create(:name=>'おもちゃ・ホビー・グッズ')
...
carbike = Category.create(:name=>'自動車・オートバイ')
...
babykid_child_array = ['ベビー服(女の子用) ~95cm','ベビー服(男の子用) ~95cm','ベビー服(男女兼用) ~95cm','キッズ服(女の子用) 100cm~','キッズ服(男の子用) 100cm~','キッズ服(男女兼用) 100cm~','キッズ靴','子ども用ファッション小物','おむつ/トイレ/バス','外出/移動用品','授乳/食事','ベビー家具/寝具/室内用品','おもちゃ','行事/記念品','その他']
...
toyhobby_child_array = ['おもちゃ','タレントグッズ','コミック/アニメグッズ','トレーディングカード','フィギュア','楽器/器材','コレクション','ミリタリー','美術品','アート用品','その他']
cosme_child_array = ['ベースメイク','メイクアップ','ネイルケア','香水','スキンケア/基礎化粧品','ヘアケア','ボディケア','オーラルケア','リラクゼーション','ダイエット','その他']
electrical_child_array = ['スマートフォン/携帯電話','スマホアクセサリー','PC/タブレット','カメラ','テレビ/映像機器','オーディオ機器','美容/健康','冷暖房/空調','生活家電','その他']
...
carbike_child_array = ['自動車本体','自動車タイヤ/ホイール','自動車パーツ','自動車アクセサリー','オートバイ車体','オートバイパーツ','オートバイアクセサリー']
...
interior_grandchild_array = [
  ...
  ['布団/毛布','枕','シーツ/カバー','その他']
  ['ごみ箱','ウェルカムボード','オルゴール','クッション','クッションカバー','スリッパラック','ティッシュボックス','バスケット/かご','フォトフレーム','マガジンラック','モビール','モビール','花瓶','灰皿','傘立て','小物入れ','置時計','掛時計/柱時計','鏡(立て掛け式)','鏡(壁掛け式)','置物','風鈴','植物/観葉植物','その他']
  ['正月','成人式','バレンタインデー','ひな祭り','子どもの日','母の日','父の日','父の日','サマーギフト/お中元','夏/夏休み','ハロウィン','敬老の日','七五三','お歳暮','クリスマス','冬一般','その他']
]
bookmusicgame_grandchild_array = [
  ['文学/小説','人文/社会','ノンフィクション/教養','地図/旅行ガイド','ビジネス/経済','ビジネス/経済','健康/医学','コンピュータ/IT','趣味/スポーツ/実用','住まい/暮らし/子育て','アート/エンタメ','洋書','絵本','参考書','その他']
...
lady_child_array.each_with_index do |child, i|
  child = lady.children.create(name: child)
...
man_child_array.each_with_index do |child, i|
  child = man.children.create(name: child)
  man_grandchild_array[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end

babykid_child_array.each_with_index do |child, i|
  child = babykid.children.create(name: child)
  babykid_grandchild_array[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end
...
bookmusicgame_child_array.each_with_index do |child, i|
  child = bookmusicgame.children.create(name: child)
  bookmusicgame_grandchild_array[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end

toyhobby_child_array.each_with_index do |child, i|
  child = toyhobby.children.create(name: child)
  toyhobby_grandchild_array[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end

cosme_child_array.each_with_index do |child, i|
  child = cosme.children.create(name: child)
  cosme_grandchild_array[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end

electrical_child_array.each_with_index do |child, i|
  child = electrical.children.create(name: child)
  electrical_grandchild_array[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end

sport_child_array.each_with_index do |child, i|
  child = sport.children.create(name: child)
...
ticket_child_array.each_with_index do |child, i|
  child = ticket.children.create(name: child)
  ticket_grandchild_array[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end

carbike_child_array.each_with_index do |child, i|
  child = carbike.children.create(name: child)
...
other_child_array.each_with_index do |child, i|
  child = other.children.create(name: child)
